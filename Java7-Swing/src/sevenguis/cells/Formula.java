...
class Application extends Formula {
    ...
    private static Map<String, Op> opTable = new HashMap<String, Op>();
    static {
        opTable.put("add", new Op() {
            public double eval(List<Double> vals) {
                return vals.get(0) + vals.get(1);
            }
        });
        opTable.put("sub", new Op() {
            public double eval(List<Double> vals) {
                return vals.get(0) - vals.get(1);
            }
        });
        opTable.put("div", new Op() {
            public double eval(List<Double> vals) {
                return vals.get(0) / vals.get(1);
            }
        });
        opTable.put("mul", new Op() {
            public double eval(List<Double> vals) {
                return vals.get(0) * vals.get(1);
            }
        });
        opTable.put("mod", new Op() {
            public double eval(List<Double> vals) {
                return vals.get(0) % vals.get(1);
            }
        });
        opTable.put("sum", new Op() {
            public double eval(List<Double> vals) {
                double accum = 0;
                for (Double i : vals) {
                    accum += i;
